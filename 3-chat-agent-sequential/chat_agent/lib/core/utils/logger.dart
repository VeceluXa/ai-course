...
class AppLogger {
  ...
  void logJson(String label, Object? value) {
    if (!kDebugMode) return;
    final encoder = const JsonEncoder.withIndent('  ');
    final payload = encoder.convert(value);
    developer.log('$label\n$payload', name: 'openai_chat_app.json');
    debugPrint('$label\n$payload');
  }

  void logJsonString(String label, String raw) {
    if (!kDebugMode) return;
    try {
      final decoded = jsonDecode(raw);
      logJson(label, decoded);
    } catch (_) {
      logWarning('$label (invalid JSON payload)');
      debugPrint(raw);
    }
  }
